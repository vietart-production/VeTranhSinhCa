...
`QRFolderScanner` also spawns a fixed "default fish" school at `Start()` from ready-made textures in `defaultFishFolderPath` (`Assets/default_fish`) — these skip QR decoding entirely and use `FindTemplateFromFileName` to match a filename to a template instead (see `Assets/default_fish/README.txt` for the naming convention, e.g. `ca_heo_01.png`).
